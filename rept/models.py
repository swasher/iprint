...
class Customer(models.Model):
    ...
    def __str__(self):
        if self.fio:
            return '{} ({} {})'.format(self.name, self.fio, self.phone)
        else:
...
class PrintingPress(models.Model):
    name = models.CharField(max_length=50, verbose_name='Наименование', help_text='Название печатного пресса')
    plate_width = models.IntegerField(verbose_name='Ширина пластины, мм')
    plate_height = models.IntegerField(verbose_name='Высота пластины, мм')
...
class PrintSheet(models.Model):
    ...
    outputter = models.ForeignKey(Manufacturer, blank=True, null=True, verbose_name='Вывод пластин')
...
class Operation(models.Model):
    ...
    manufacturer = models.ForeignKey(Manufacturer, blank=True, null=True, verbose_name='Подрядчик')
    price = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True, verbose_name='Стоимость, грн.')
